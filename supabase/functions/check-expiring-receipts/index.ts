  ...
  try {
    ...
    for (const userPref of userPreferences || []) {
      try {
        ...
        // Calculate the notification date (today + notify_days_before should equal due_date)
        const notificationDate = new Date(today);
        notificationDate.setDate(today.getDate() + userPref.notify_days_before);

        // Get user's receipts that are due on the notification date
        const { data: receipts, error: receiptsError } = await supabase
          .from('receipts')
          .select('*')
          .eq('user_id', userPref.user_id)
          .eq('due_date', notificationDate.toISOString().split('T')[0])
